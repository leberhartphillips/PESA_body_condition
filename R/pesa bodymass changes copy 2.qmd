...
ODBA_23 %>% 
  ...
  ggplot(.) +
  geom_line(aes(x = timestamp, y = ODBA), color = "gray70") +
...
cap_05_09_23_std_pca_delta_odba <- 
  ...
  group_by(capture_id) %>% 
  mutate(daily_sum_ODBA = sum_ODBA/date_deviance,
         daily_delta_weight = delta_weight/date_deviance)

cap_05_09_23_std_pca_delta_odba %>% 
  dplyr::select(year_, capture_id, weight_1, first_date, 
                date_deviance, delta_weight, daily_sum_ODBA, 
                daily_delta_weight, avg_ODBA) %>% 
  datatable(class = 'cell-border stripe', rownames = FALSE, filter = 'top')
# cap_05_09_23_std_pca_delta_odba %>% 
#   arrange(daily_sum_ODBA) %>% 
#   dplyr::select(capture_id, tagID, daily_delta_weight, daily_sum_ODBA, weight_1, first_date)
```
...
- significant negative effect of first capture date on change in mass (late arriving males experience more mass change over time)
```{r}
tbl_regression(mod_avgODBA_delta, intercept = TRUE)
