...
cap_05_09_23_std_pca_delta_odba %>% 
  dplyr::select(year_, capture_id, weight_1, weight_1_, first_date, first_date_, 
                ...
                daily_delta_weight, avg_ODBA, avg_ODBA_, gm_ODBA, gm_ODBA_) %>% 
...
model assessing the relationship between geometric mean ODBA and change in body mass (converted to daily rate to account for varied time windows between body mass measures). Note that all variables are standardized.
```{r, echo=TRUE}
# non-standardized model
mod_gmODBA_delta <-
...
cap_05_09_23_std_pca_delta_odba %>% 
  ungroup() %>% 
  dplyr::select(gm_ODBA_, weight_1_, first_date_) %>% 
  cor() %>% 
  corrplot(type = "upper", method = "number", tl.srt = 45)
```
